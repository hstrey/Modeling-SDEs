...
function twoD_Gaussian(xy, p)
    offset, amplitude, xo, yo, sigma_x, sigma_y, rho = p

    # creating linear meshgrid from xy
    x = xy[:, 1]
    y = xy[:, 2]
    g = offset .+ amplitude .* (((x .- xo).^2)/sigma_x^2 - 2 .* rho .* (x .- xo)/sigma_x .* (y .- yo)/sigma_y + ((y .- yo).^2)/sigma_y^2)
    return g[:]
end
...
function twoDG(x,y,p)
    ...
    # creating linear meshgrid from xy
    return offset + amplitude * (((x - xo)^2)/sigma_x^2 - 2 * rho * (x - xo)/sigma_x * (y - yo)/sigma_y + ((y - yo)^2)/sigma_y^2)
end

p0 = Float64.([-10.0, 3, 0.5, 0.18, 0.01, 0.01, -0.6])

fit = LsqFit.curve_fit(twoD_Gaussian, ll12, z, p0)
confidence_inter = confidence_interval(fit,0.05)
p = coef(fit)
twoDGp(x,y) = twoDG(x,y,p)
