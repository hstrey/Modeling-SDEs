...
@model function fitlv(time_mean, time_std,prob)
    ...
    for j in 2:length(time_mean)
        time_mean[j] ~ Normal(pred_mean.u[j-1],σₘ)
        time_std[j] ~ Normal(pred_std.u[j-1],σₛ)
    end
end

model = fitlv(vec(time_mean[2:end]), vec(time_std[2:end]),prob)

# This next command runs 3 independent chains without using multithreading.
chain = sample(model, NUTS(0.65), 100, init_theta = [time_mean[1],0.6678,0.0108,2.3401,0.2369,0.6929,0.6958])
#summarystats(chain)
plot(chain)
